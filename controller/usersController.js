...
exports.updateAvatar = async (req, res) => {
  ...
  if (!avatar) {
    return res
      .status(400)
      .json({ status: false, errorMessage: "Missing avatar" });
  }
  ...
  try {
    pool.query(
      ...
      (err) => {
        ...
        res
          .status(200)
          .json({ status: true, title: "Avatar updated successfully" });
      }
    );
  } catch (err) {
    res.status(500).json({ status: "fail", message: err.message });
    ...
    pool.query(
      ...
      (err, results) => {
        ...
        if (results.rows.length === 0) {
          return res
            .status(404)
            .json({ status: false, errorMessage: "User not found" });
        }
...
exports.login = async (req, res) => {
  ...
  if (!username || !password) {
    return res
      .status(400)
      .json({ status: false, errorMessage: "Missing username or password" });
  }
    ...
    pool.query(
      ...
      async (err, results) => {
        ...
        if (!results.rows.length)
          return res
            .status(400)
            .json({ status: false, errorMessage: "No user found" });
      ...
      async (err, results) => {
        ...
        pool.query(
          ...
          (err) => {
            ...
            res
              .status(200)
              .json({ status: true, title: "Password changed successfully" });
          }
        );
      }
    );
